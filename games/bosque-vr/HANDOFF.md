...
- Assets locais (CC0): `sky.hdr` (HDRI de céu, Poly Haven), `grass.jpg` (textura de grama, Poly Haven) e os modelos 3D em `models/` (`trees.glb`, `pines.glb`, `rocks.glb`, `deer.glb` — todos CC0 Quaternius/poly.pizza). `game.json` e `cover.svg` (capa) também na pasta.
...
- ~22 pedras (`models/rocks.glb`, 5 variantes, mesmo esquema de instancing, altura ~1.4 m) e ~11 AVES voando (procedurais: corpo, cabeça, bico, cauda e 2 asas que batem; voam em círculos, orientadas com `lookAt`).
- 2 VEADOS animados (`models/deer.glb`, SkinnedMesh CC0 Quaternius — **sem instancing**, é skinned): `AnimationMixer` tocando o clipe "Walk" (com as tracks `.position` removidas pra não ter root motion — quem move é o `wrap`); vagam devagar em círculos seguindo o relevo, virados com `lookAt`. A escala é aplicada no `wrap` (Group pai), não no model, pra manter o skinning estável; `frustumCulled=false` nos meshes. (No headless/swiftshader o skinned mesh pode não aparecer; renderiza em GPU real / no Quest.)
